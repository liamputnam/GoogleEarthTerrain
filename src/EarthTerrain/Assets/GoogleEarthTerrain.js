...
function get_vertex_row(start, end, index)
{
	// construct url
	...
	var elevation_data = SimpleJSON.parse(www.text);
...
function get_vertex_grid(center)
{
	...
	for (var i = 0; i < num_samples_per_row; i++)
	{
		...
		get_vertex_row([row_latitude, left_longitude], [row_latitude, right_longitude], i);
		while (vertex_grid[i] == null)
		{
			print("vertex_grid[" + i + "] not set. Waiting");
